...
import sys, os, re, shutil, json, urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse, http.server, operator
...
def decode_values(path):
    ...
    return urllib.parse.unquote(params['values']).decode('utf8')

def by_func(func, k, path):
    params = get_params(path)
    values = urllib.parse.unquote(params['values']).decode('utf8').split(' ')
    if not values:
        return {k: []}
    result = {k: list(set(reduce(operator.concat, [func(url) for url in values])))}
    return result
